...
id, op, node = expr
@test id == IDENTIFIER(7, 7, "UIColor")
@test op == OPERATOR(1, 1, Tokens.DOT, false)
@test node == EXPR{Quotenode}([IDENTIFIER(3, 3, "red")], 3, 3)
@test expr == BinarySyntaxOpCall(id, op, node)

expr = CSTParser.parse("view.addSubview(subview)")
opcall, lparen, id, rparen = expr
@test opcall.arg1 == IDENTIFIER(4, 4, "view")
@test opcall.op == OPERATOR(1, 1, Tokens.DOT, false)
@test opcall.arg2 == EXPR{Quotenode}([IDENTIFIER(10, 10, "addSubview")], 10, 10)
@test opcall == BinarySyntaxOpCall(opcall.arg1, opcall.op, opcall.arg2)
@test lparen == PUNCTUATION(Tokens.LPAREN, 1, 1)
@test id == IDENTIFIER(7, 7, "subview")
@test rparen == PUNCTUATION(Tokens.RPAREN, 1, 1)
